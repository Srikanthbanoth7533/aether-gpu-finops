...
class UserActivityLog(Base):
    ...
    action = Column(String, nullable=False)   # "login", "simulate", "export", etc.
...
class GPUClusterNode(Base):
    ...
    provider = Column(String, nullable=False)         # AWS, GCP, On-Prem
    region = Column(String, nullable=False)           # us-east-1, us-central1, private-dc
    gpu_type = Column(String, nullable=False)         # H100, A100, L4
...
class ModelDeployment(Base):
    ...
    id = Column(String, primary_key=True, index=True) # e.g. llama-3-70b-service
    name = Column(String, nullable=False)             # Llama-3-70B-Instruct
    node_id = Column(String, ForeignKey("gpu_cluster_nodes.id"), nullable=False)
